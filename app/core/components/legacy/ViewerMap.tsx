...
import { useParentSize } from 'app/core/hooks/useParentSize'
import { useWikiData } from 'app/core/hooks/useWiki'
import { multiPolygonToPolygon, invertColor } from 'app/core/util/constants'

const GlobeTmpl = dynamic(() => import('../../util/GlobeWrapper'), {
  ssr: false,
})

const Globe = forwardRef((props: GlobeProps, ref) => <GlobeTmpl {...props} forwardRef={ref} />)
...
const MapContainer = ({ year, fullscreen, user, id, threeD = true }: MapContainerProps) => {
          ...
          {data && (
            <>
              <Globe
                ...
                polygonsData={
                  multiPolygonToPolygon(data.borders as FeatureCollection<MultiPolygon>).features
                }
                ...
                labelColor={d => invertColor((d as Feature<Point>).properties?.COLOR, true)}
                labelSize={d => {
                  return size(parseFloat((d as Feature<Point>).properties?.AREA))
                }}
                labelAltitude={0.015}
